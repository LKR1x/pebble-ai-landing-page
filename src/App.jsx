...
import BunnyRive from "./components/BunnyRive"; // This import will work in your local project
...
export default function App() {
  ...
  // THE FIX: State to detect if it's a touch-only device
  ...
  useEffect(() => {
    ...
    // THE FIX: Check for touch-only device once on mount
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-50 text-gray-800 relative overflow-hidden antialiased"> {/* Subtle gradient for premium depth */}

      {/* ===== Header ===== */}
      <header className="absolute top-0 left-0 right-0 z-50 p-4 md:p-6 bg-white/80 backdrop-blur-md shadow-sm"> {/* Glassmorphism for premium header */}
        <div className="container mx-auto flex justify-between items-center">
           <a href="#" className="text-xl font-bold tracking-widest font-pixeloid text-red-600 z-50 hover:text-red-700 transition-colors duration-300">PEBBLE</a> {/* Slightly darker red on hover */}
      ...
      {/* ===== Mobile Menu Overlay ===== */}
      <div className={`fixed top-0 right-0 h-full w-full z-40 bg-white/95 backdrop-blur-md p-8 pt-24 transition-all duration-500 ease-in-out md:hidden ${isMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}> {/* Added opacity transition for smoother feel */}
        <div className="flex flex-col">
          {menuLinks.map((link, index) => (
            <React.Fragment key={link.name}>
              ...
              {index < menuLinks.length - 1 && <div className="w-full border-t border-red-600/10">{/* Softer border */}</div>}
            </React.Fragment>
          ))}
        </div>
      </div>
      ...
      <div className="hidden md:block w-full h-screen relative">
        {/* This new wrapper nudges the entire content block up for better balance */}
        <div className="relative w-full h-full -translate-y-10">
            <h1 className="absolute inset-0 flex items-center justify-center font-necosmic text-[22vw] lg:text-[20vw] font-black text-red-600 leading-none tracking-tighter pointer-events-none drop-shadow-md"> {/* Added subtle shadow */}
              PEBBLE
            </h1>
            <div 
              ...
              style={{
                ...
                transition: 'transform 0.6s cubic-bezier(0.16, 1, 0.3, 1)' // Smoother bezier for premium animation
              }}
            >
               <div className="w-[360px] h-[360px] lg:w-[400px] lg:h-[400px] drop-shadow-2xl"> {/* Enhanced shadow for depth */}
                  <BunnyRive src="/pebble.riv" stateMachine="pebble" width={400} height={400} isTouchDevice={isTouchDevice} />
              </div>
            </div>
            ...
            <div className="absolute bottom-16 left-0 right-0 z-10 flex flex-col items-center">
                <a href="#" className="font-pixeloid bg-gradient-to-r from-red-600 to-red-700 text-white font-bold py-4 px-10 rounded-full text-lg flex items-center gap-3 hover:scale-105 transition-all duration-300 shadow-xl shadow-red-600/40 hover:shadow-red-600/60"> {/* Gradient button for premium look */}
                    SAY HELLO
                </a>
      ...
      <div className="md:hidden w-full min-h-screen flex flex-col items-center justify-center text-center px-6 pt-24 pb-20">
          <div className="w-[240px] h-[240px] drop-shadow-2xl"> {/* Enhanced shadow */}
              <BunnyRive src="/pebble.riv" stateMachine="pebble" width={240} height={240} isTouchDevice={isTouchDevice} />
          </div>
      ...
      {/* ===== Footer ===== */}
      <footer className="absolute bottom-0 left-0 right-0 p-4 z-20 bg-white/80 backdrop-blur-md shadow-sm"> {/* Glassmorphism footer */}
